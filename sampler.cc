...
void GetNonUniformSample(const ValueType* probability,
  ...
  for (size_t i = 0; i < ver_len; ++i) {
    sp_prob[i] = probability[edge_id_list[i]]/100000;
  }
...
NodeFlow SampleSubgraph(const ImmutableGraph *graph,
  ...
  for (int layer_id = 1; layer_id < num_hops; layer_id++) {
    //num_neighbor[0] controls sampling method,when 1,layer sampling follows are
    //num_neighbor of each layer
    ...
    for (size_t idx = layer_offsets[layer_id - 1]; idx < layer_offsets[layer_id]; idx++) {     
      //make a var 'i'(i start from 1) and read num_neighbor[i] as max num of neighbor    
      ...
      //TODO:
      input_num_neighbor = num_neighbor[layer_id-1];
...
NodeFlow SamplerOp::NeighborSample(const ImmutableGraph *graph,
                                   const std::vector<dgl_id_t>& seeds,
                                   const std::string &edge_type,
                                   int num_hops, std::vector<int64_t>& expand_factor,
                                   const bool add_self_loop,
                                   const ValueType *probability) {
...
namespace {
  void ConstructLayers(const dgl_id_t *indptr,
                       ...
                       std::vector<int64_t>& probabilities) {
    /*
    ...
    for (int64_t i = num_layers - 1; i >= 0; --i) {
      ...
      std::vector<size_t> sp_index(layer_size);
      std::copy(candidate_set.begin(), candidate_set.end(),
                std::back_inserter(candidate_vector));
      
      std::vector<float> candidate_prob(candidate_vector.size());
      for (size_t j = 0; j < candidate_vector.size() ; ++ j) {
        // float temp = probabilities[candidate_vector[j]];
        candidate_prob[j] = probabilities[candidate_vector[j]];
      }
  
      ArrayHeap<float> arrayHeap(candidate_prob);
      arrayHeap.SampleWithoutReplacement(layer_size, &sp_index);
      
      std::unordered_map<dgl_id_t, size_t> n_occurrences;
      auto n_candidates = candidate_vector.size();
      
      for (int64_t j = 0; j != layer_size; ++j) {
        int dst = candidate_vector[sp_index[j]];
...
std::vector<NodeFlow> NeighborSamplingImpl(const ImmutableGraphPtr gptr,
                                           ...
                                           const IdArray expand_factor,
                                           const int64_t num_hops,
                                           const std::string neigh_type,
                                           const bool add_self_loop,
                                           const ValueType *probability) {
    // process args
    CHECK(aten::IsValidIdArray(expand_factor));
    const int64_t* expand_factor_data = static_cast<int64_t*>(expand_factor->data);
    const int64_t num_nn = expand_factor->shape[0];
    std::vector<int64_t> num_neighbor(num_nn);
    std::copy(expand_factor_data, expand_factor_data + num_nn,
                num_neighbor.begin());
...
#pragma omp parallel for
    for (int i = 0; i < num_workers; i++) {
      ...
      nflows[i] = SamplerOp::NeighborSample(
          gptr.get(), worker_seeds, neigh_type, num_hops, num_neighbor,
...
.set_body([] (DGLArgs args, DGLRetValue* rv) {
    ...
    const IdArray expand_factor = args[5];
...
.set_body([] (DGLArgs args, DGLRetValue* rv) {
    ...
    CHECK(aten::IsValidIdArray(expand_factor));
    CHECK_EQ(expand_factor->ctx.device_type, kDLCPU)
      << "NeighborSampler only support CPU sampling";
...
.set_body([] (DGLArgs args, DGLRetValue* rv) {
    ...
    const IdArray probabilities =args[7];
    // process args
    auto gptr = std::dynamic_pointer_cast<ImmutableGraph>(g.sptr());
    CHECK(gptr) << "sampling isn't implemented in mutable graph";
    CHECK(aten::IsValidIdArray(seed_nodes));
    ...
    CHECK(aten::IsValidIdArray(probabilities));
    CHECK_EQ(probabilities->ctx.device_type, kDLCPU)
      << "LayerSampler only support CPU sampling";
    const int64_t* probabilities_data = static_cast<int64_t*>(probabilities->data);
    const int64_t num_prob = probabilities->shape[0];
    std::vector<int64_t> prob(num_prob);
    std::copy(probabilities_data, probabilities_data + num_prob,
                prob.begin());
...
#pragma omp parallel for
    for (int i = 0; i < num_workers; i++) {
      ...
      nflows[i] = SamplerOp::LayerUniformSample(
          gptr.get(), worker_seeds, neigh_type, layer_sizes, prob);
    }
    *rv = List<NodeFlow>(nflows);
  });
