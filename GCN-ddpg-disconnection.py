...
#################################################################################################################################
        ...
        for key, value in P[0].items():  
            for i in range(self.num_clients):
                if i == 0:
                    P[0][key] = P[i][key] * (len(self.Client[i]) / self.g_train.number_of_nodes())
                else:
                    P[0][key] += P[i][key] * (len(self.Client[i]) / self.g_train.number_of_nodes())
...
def runGraph(Model,Graph, Local_Graph,args,Optimizer,Labels,train_nid,norm_local,features_local,train_mask_local,cuda,sampling):
    ...
    subGraph, seedIdx = DelNonLocalEdges(Graph, Local_Graph, NodeFlow)

    NF = []
    for idx in range (len(seedIdx)):
        nfs = dgl.contrib.sampling.NeighborSampler(subGraph[idx], args.batch_size,
                                                            expand_factor = sampling,
                                                            neighbor_type='in',
                                                            shuffle=True,
                                                            num_workers=16,
                                                            num_hops=args.n_layers+1,
                                                            seed_nodes=seedIdx[idx])
        NF.append(nfs)
        subGraph[idx].to(torch.device('cuda:0'))
    for NF_objects in NF:
